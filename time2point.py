...
class DayMode(object):
    ...
    def __init__(self):
        ...
        for i in range(self.CFFEX.morning_begin, self.CFFEX.morning_end):
            ...
            self.cffex_point2time[count] = (i / 2) * 1000 + (
                i % 2) * self.INTERVAL + self.BEGIN_MILLI_SEC
            count += 1

        for i in range(self.CFFEX.afternoon_begin,
                       self.CFFEX.afternoon_end + 2):
            self.cffex_time2point[i] = count
            self.cffex_point2time[count] = (i / 2) * 1000 + (
                i % 2) * self.INTERVAL + self.BEGIN_MILLI_SEC
            count += 1
        ...
        for i in range(self.OtherBreak.morning_begin,
                       self.OtherBreak.morning_break_begin):
            self.other_time2point[i] = count
            self.other_point2time[count] = (i / 2) * 1000 + (
                i % 2) * self.INTERVAL + self.BEGIN_MILLI_SEC
            count += 1

        for i in range(self.OtherBreak.morning_break_end,
                       self.OtherBreak.morning_end):
            self.other_time2point[i] = count
            self.other_point2time[count] = (i / 2) * 1000 + (
                i % 2) * self.INTERVAL + self.BEGIN_MILLI_SEC
            count += 1

        for i in range(self.OtherBreak.afternoon_begin,
                       self.OtherBreak.afternoon_end + 2):
            self.other_time2point[i] = count
            self.other_point2time[count] = (i / 2) * 1000 + (
                i % 2) * self.INTERVAL + self.BEGIN_MILLI_SEC
            count += 1
