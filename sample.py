    ...
    for id,tick_df in tick_df_all.groupby('id'):
        ...
        for tick_spot in xrange(0,total_spots_tick - spots_gap,spots_gap):
            try:
                ...
                min_df.ix[min_spot,'Time'] = tick_df.ix[tick_spot,'Time'].split('.')[0]
                min_df.ix[min_spot,'OpenPrice'] = float(tick_df.ix[tick_spot,'LastPrice'])
                min_df.ix[min_spot,'HighPrice'] = float(tick_df.ix[tick_spot:tick_spot+spots_gap-1,'LastPrice'].max())
                min_df.ix[min_spot,'LowPrice'] = float(tick_df.ix[tick_spot:tick_spot+spots_gap-1,'LastPrice'].min())
                min_df.ix[min_spot,'ClosePrice'] = float(tick_df.ix[tick_spot+spots_gap-1,'LastPrice'])
                min_df.ix[min_spot,'Volume'] = int(tick_df.ix[tick_spot+spots_gap-1,'Volume'])
                min_df.ix[min_spot,'OpenInterest'] = int(tick_df.ix[tick_spot+spots_gap-1,'OpenInterest'])
            except:
                print tick_spot
